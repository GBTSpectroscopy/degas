...
for (galaxy,color,marker) in zip(degas[dr1],colorlist,markerlist):
    ...
    ax.errorbar(molmass[~lolims], fdense[~lolims],
                yerr = fdense_err[~lolims],
                marker = marker,
                linestyle= '--',
                color=color,
                label=galaxy['NAME'])
    ...
    ax.set_xlabel(r'$\Sigma_{*}$ (M$_{mol}$ pc$^{-2}$)')
    ax.set_ylabel(r'log$_{10}$ (HCN-to-CO)')

    fig.show()

fig.savefig(os.path.join(plotDir,'fdense_vs_molmass_combined_nolim.pdf'))    
fig.savefig(os.path.join(plotDir,'fdense_vs_molmass_combined_nolim.png'))    
