...
for (galaxy,color,marker) in zip(degas[dr1],colorlist,markerlist):
    ...
    ax.scatter(r25[~lolims], fdense[~lolims],
                marker=marker,
                color=color,
                label=galaxy['NAME'])
...
fig.savefig(os.path.join(plotDir,'fdense_vs_r25_combined.pdf'))    
fig.savefig(os.path.join(plotDir,'fdense_vs_r25_combined.png'))    
